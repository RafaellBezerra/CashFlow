...
namespace WebApi.Tests.Expenses.Delete
{
    public class DeleteExpenseTest : CashFlowClassFixture
    {
        ...
        public DeleteExpenseTest(IntegrationTestServer testServer) : base(testServer)
        {
        ...
        {
            var response = await DoDelete(requestUri: $"{PATH}/{_expenseId}", token: _token);

            response.StatusCode.ShouldBe(HttpStatusCode.NoContent);

            response = await DoGet(requestUri: $"{PATH}/{_expenseId}", token: _token);
